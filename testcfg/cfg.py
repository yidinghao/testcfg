"""
This module contains an implementation of CFGs. A CFG is
represented by the class ContextFreeGrammar. Nonterminals and
terminals are represented as strings. A production is represented
as a tuple (lhs, rhs), where lhs is a nonterminal and rhs is a
tuple of nonterminals and terminals.
...
class ContextFreeGrammar:
    """
    This class represents an arbitrary context-free grammar. We assume
    that the set of nonterminals is disjoint from the set of terminals.
    ...
    def all_terminals(self, rhs):
        """
        Checks to see if all symbols in the right-hand side of a production
        are terminals.

        :type rhs: tuple
        :param rhs: A tuple of symbols.

        :rtype: bool
        :return: True if all elements of rhs are terminals; False otherwise.
        """
        for x in rhs:
    ...
    def is_empty(self):
        """
        Checks to see whether this grammar is empty.

        :rtype: bool
        :return: True if this grammar does not have any nonterminals;
            False otherwise.
        """
    ...
    def number_terminal_slots(self):
        """
        Counts the number of times a terminal appears on the right-hand
        side of a production.

        :rtype: int
        :return: The number of times a terminal appears on the right-hand
            side of a production.
        """
        n = 0
    ...
    def compute_coreachable(self):
        """
        Finds all the nonterminals that generate a string.

        :rtype: set
        :return: The set of all nonterminals that generate a string.
        """
        coreachable = set()
        iteration = 0
        done_this_loop = 0
    ...
    def compute_nonnulling(self):
        """
        Finds all the nonterminals that generate a nonempty string.

        :rtype: set
        :return: The set of all nonterminals that generate a nonempty
            string.
        """
    ...
    def _check_tuple(self, tup, symbol_set):
        """
        Check to see if every element of a given tuple is in a given set.

        :type tup: tuple
        :param tup: A tuple.

        :type symbol_set: set
        :param symbol_set: A set that elements of tup should come from.

        :rtype: bool
        :return: True if every element of tup is in symbol_set; False
            otherwise.
        """
        for i in tup:
    ...
    def compute_nulling(self):
        """
        Finds all the nonterminals that only generate the empty string.

        :rtype: set
        :return: The set of all nonterminals only generate the empty string.
        """
    ...
    def compute_nullable(self):
        """
        Finds all the nonterminals that generate the empty string.

        :rtype: set
        :return: The set of all nonterminals that generate the empty string.
        """
        nullable = set()
        for prod in self.productions:
            if len(prod[1]) == 0:
    ...
    """ Methods for intersecting a grammar with a regular language """
    ...
    def dump(self):
        """
        Prints a description of this object to the terminal.
    ...
    """
        In the original paper of Clark, it was pointed out that most of
        the grammars generated had the 1-FKP because rules of the form
        A -> a, where a is a nonterminal, guaranteed that {a} would be
        a 1-kernel for A. The following code tests whether the 1-FKP would
        still be common without such rules. Given a CFG in CNF, lexical
        productions are removed using the following algorithm.

        - For each lexical production A -> a:
            - For each production B -> B1 B2 ... Bn:
                - If Bi = A for some i, then add the production
                    B -> B1 B2 ... B(i - 1) a B(i + 1) ... Bn.
            - Remove the production A -> a.
    """

    def is_lexical_rule(self, prod):
        """
        Checks to see if a production is of the form A -> a,
        ...
        :rtype: bool
        :return: True if the production is of the form A -> a;
            False otherwise.
        """
        if len(prod[1]) != 1:
            return False
        return prod[1][0] in self.terminals

    def remove_lexical_rules(self):
        """
        This method removes all rules of the form A -> a, where a
        is a nonterminal. For every rule in which A appears on the RHS,
        a duplicate rule will be created with A replaced by a.
